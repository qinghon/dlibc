...
pub const LINUX_HEADERS: &[&str] = &[
	...
	"linux/mctp.h",
	"linux/mdio.h",
	"linux/media/raspberrypi/pisp_be_config.h",
	"linux/media/raspberrypi/pisp_common.h",
	...
	// depend syms loss
	// "xen/gntdev.h",
	// depend "xen/interface/xen.h" loss
	// "xen/privcmd.h",
];

pub fn get_header_dependencies() -> HashMap<&'static str, &'static [&'static str]> {
	let mut header_map: HashMap<&'static str, &[&'static str]> = HashMap::new();
	for headers in NEED_DEPENDENCIES_HEADERS {
		header_map.insert(headers[0], &headers[1..]);
	}
	header_map
}

const NEED_DEPENDENCIES_HEADERS: &[&[&str]] = &[
	&["linux/coda.h", "sys/types.h"],
	&["linux/errqueue.h", "sys/time.h"],
	&["linux/hdlc/ioctl.h", "net/if.h"],
	&["linux/omapfb.h", "stddef.h"],
	&["linux/phonet.h", "sys/socket.h"],
	&["linux/sctp.h", "stdint.h", "sys/socket.h"],
	&["linux/sysctl.h", "stddef.h"],
	&["linux/usb/audio.h", "stddef.h"],
	&["linux/vm_sockets.h", "sys/socket.h"],
	&["xen/evtchn.h", "sys/socket.h"],
];
